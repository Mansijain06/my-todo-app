...
export default function TodoList(props) {
  ...
  const addItem = () => {
    ...
    fetch("https://todo-app06-backend.herokuapp.com/todo", {
      method: "POST",
  ...
  const deleteHandler = (itemIdx) => {
    ...
    fetch(`https://todo-app06-backend.herokuapp.com/todo/${idToDelete}`, {
      method: "DELETE",
      credentials: "include",
    }).then((r) => {
  ...
  const editHandler = (editedValue, itemIdx) => {
    ...
    fetch(`https://todo-app06-backend.herokuapp.com/todo/${idToEdit}`, {
      method: "PUT",
